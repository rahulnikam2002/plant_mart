...
const EditProductPage = () => {
    ...
    const [productImgs, setProductImgs] = useState({
        1: {
            img: productDetails.featuredImages[0],
            raw: productDetails.featuredImages[0]
        },
        2: {
            img: productDetails.featuredImages[1],
            raw: productDetails.featuredImages[1]
        },
        3: {
            img: productDetails.featuredImages[2],
            raw: productDetails.featuredImages[2]
        }
    });
    ...
    const handleUpdateProduct = async () => {
        try {
            ...
            if (validateData) {
                ...
                console.log({ productImgs, productDetails });
                for (imagess in productImgs) {
                    console.log(imagess);
                }
    ...
    //     try {
    //        const productData = await axios.get(`/api/products/search/id/${}`)
    //     } catch (error) {
    //         console.log(error)
    ...
    const handleLoadProductData = useCallback(async () => {
        try {
            const productData = await axios.get(`/api/products/search/id/${productSlug}`);
            setProductDetails(productData.data);
            console.log(productData.data);
            // Set initial state of productImgs
            const featuredImages = productData.data.featuredImages;
            console.log({ featuredImages });
            setProductImgs({
                1: { raw: featuredImages[0] },
                2: { raw: featuredImages[1] },
                3: { raw: featuredImages[2] }
            });
        } catch (error) {
            console.log(error);
            errorToast("Something went wrong");
