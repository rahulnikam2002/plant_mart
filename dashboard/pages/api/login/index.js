...
export default async function handler(req, res) {
    ...
    if (validator.isEmail(userName)) {
        ...
        return res.status(400).send({
            code: 0,
            msg: "Something went wrong, make sure you enter correct credentials"
        });
    }

    try {
        const getAdminDetails = await adminModel.findOne(query).select(query.emailId ? "emailId password" : "phone password");
        ...
        if (password === getAdminDetails.password) {
            ...
            return res.status(401).json({
                valid: false,
                msg: validator.isEmail(userName) ? "Email or password incorrect" : "Phone or password incorrect"
            });
        }
    } catch (error) {
