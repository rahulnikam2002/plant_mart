...
export const PasswordScreen = ({ route, navigation }) => {
    ...
    const handleSignIn = async (email, password) => {
        // sign in code
        setLoading(true);
        const response = await axios.post(`${networkIP}/api/user/authentication/signin`, { email, password });
    ...
    const handleSignUp = async (email, password, key) => {
        // sign up code
        setLoading(true);
        const response = await axios.post(`${networkIP}/api/user/authentication/signup/`, { email, password, key, userName, phoneNumber });
        const responseObject = response.data;
        setLoading(false);
        if (responseObject.type !== "SUCCESS") {
            return handleShowToasts(responseObject.code);
    ...
    return (
        <CustomSafeAreaView style={{ height: "100%" }}>
            ...
            <HeaderCard
                ...
                text={authMode === "signup" ? "Create secure password and Details to continue" : "Enter your password to continue"}
